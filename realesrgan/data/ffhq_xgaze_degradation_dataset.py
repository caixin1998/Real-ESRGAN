...
class FFHQXGazeDegradationDataset(data.Dataset):
    def __init__(self, opt) -> None:
        super(FFHQXGazeDegradationDataset).__init__()

        self.xgaze_dataset = XGazeDegradationDataset(opt)
        self.ffhq_dataset = FFHQDegradationDataset(opt)
        self.len_xgaze = len(self.xgaze_dataset)
        self.len_ffhq = len(self.ffhq_dataset)
    def __getitem__(self, index):
        if random.randint(0,1):
            return self.ffhq_dataset[index % (self.len_ffhq)]
        else:
            return self.xgaze_dataset[index % (self.len_xgaze)]
    def __len__(self):
        return max(self.len_ffhq, self.len_xgaze)
...
class FFHQDegradationDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.gt_folder = opt['dataroot_gt_ffhq']
        self.mean = opt['mean']
        self.std = opt['std']
        self.out_size = opt['out_size']
